...
class gene(catalog_object):
    ...
    def matches(db, text):
        ...
        ret = query.response(
            db,
            "SELECT gene FROM genes WHERE gene='"+text+"'")
        return ret.nrow() > 0
    def retrieve_where(value):
...
class region(catalog_object):
    ...
    def retrieve_object(db, value, pthreshold):
        ...
        ret = query.singleton_response(
            db,
            "SELECT COUNT(DISTINCT gene) FROM cpgs WHERE " + where)
        genes = ret.value()
        ret = query.singleton_response(
            db,
            "SELECT COUNT(DISTINCT cpg) FROM cpgs WHERE " + where)
        sites = ret.value()
        ret = query.singleton_response(
            db,
...
class efo_term(catalog_object):
    ...
    def retrieve_object(db, value, pthreshold):
        ...
        ret = query.response(
            db,
            "SELECT efo, pubs, ifnull(assocs,0) as assocs "
            + "FROM efo_terms where efo='"+value+"'")
        if ret.nrow() > 0:
...
class study(catalog_object):
    def __init__(self, name, author, pmid, trait, tissue, array, n, assocs):
        ...
        self.add_detail('methylation_array', array)
        self.add_detail('n', n)
    def extract_value(text):
        return catalog_object.extract_value(text)
    def matches(db, text):
    ...
    def retrieve_matching(db, where, pthreshold, sig=False):
        if not sig:
            ret = query.response(
                db,
                "SELECT study_id, n, pmid, author, "
                + "trait, tissue, methylation_array, "
            ...
            ret = query.response(
                ...
                + "n, pmid, author, trait, tissue, methylation_array, "
        ...
        return [study(ret.element("study_id", i),
                      ...
                      ret.element("methylation_array", i),
                      ret.element("n", i),
                      ret.element("assocs", i))
                for i in range(ret.nrow())]
    def retrieve_suggestions(db, value, pthreshold):
...
class trait(catalog_object):
    ...
    def retrieve_traits(db, value, pthreshold):
        ...
        ret = query.response(
            db,
            "SELECT DISTINCT traits.trait as trait, "
            #+ "studies.efo as efo, "
