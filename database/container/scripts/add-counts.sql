...
-- reset date of last update
delete from last_update;
insert into last_update (update_date) value (CURDATE());
...
update genes left join (
  select gene, count(distinct study_id,results.cpg) as n
  from results
  join cpgs on results.cpg = cpgs.cpg
  where p < @threshold
  group by gene
